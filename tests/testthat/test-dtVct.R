if (identical(Sys.getenv("NOT_CRAN"), "true") & .Machine$sizeof.pointer != 4){
  ...
  test_that("dtVct_CINTheterogeneity", {
    ...
    for (subi in 1:nsubjects) {
      gm <- ctModel(LAMBDA = diag(1), Tpoints = Tpoints, DRIFT = matrix(-1),
                    T0MEANS = matrix(4), CINT = matrix(par[subi]),
                    DIFFUSION = matrix(2), T0VAR = matrix(2),
                    MANIFESTVAR = matrix(.8))
      d <- suppressMessages(ctGenerate(gm, n.subjects = 1, burnin = 3,
                                       wide = FALSE, dtmean = dt))
      if (subi == 1) dat <- cbind(subi, d) else dat <- rbind(dat, cbind(subi, d))
    }

    colnames(dat)[1] <- "id"

    cm <- ctModel(LAMBDA = diag(1), type = "stanct", CINT = matrix("cint"),
    ...
    dm <- ctModel(LAMBDA = diag(1), type = "standt", CINT = matrix("cint"),
                  MANIFESTMEANS = matrix(0))
    ...
    for (m in c("cm", "dm")) {
      argslist <- list(
        ml = list(datalong = dat, ctstanmodel = get(m), optimize = TRUE,
                  nlcontrol = list(), verbose = 0,
                  optimcontrol = list(plotsgd = F, estonly = F, stochastic = F),
    ...
    dtpars <- lapply(s, function(argi) {
                         ct <- argi$cm$parmatrices[c(grep("(^dt|^asym)", rownames(argi$cm$parmatrices)),
                                                     which(rownames(argi$cm$parmatrices) %in%
                                                           c("MANIFESTVAR", "LAMBDA", "T0MEANS", "T0VAR"))
                                                     ), c("Mean", "Sd")]
                         dt <- argi$dm$parmatrices[c(grep("(^asym)", rownames(argi$dm$parmatrices)),
                                                     which(rownames(argi$dm$parmatrices) %in%
                                                           c("DRIFT", "CINT", "DIFFUSIONcov", "MANIFESTVAR",
                                                             "LAMBDA", "T0MEANS", "T0VAR"))),
                                                   c("Mean", "Sd")]
                         t <- paste0("dt", rownames(dt)[rownames(dt) %in% c("DRIFT", "CINT", "DIFFUSIONcov")])
                         rownames(dt)[rownames(dt) %in% c("DRIFT", "CINT", "DIFFUSIONcov")] <- t
  ...
  test_that("dtVct_noheterogeneity", {
    ...
    for (subi in 1:nsubjects) {
      gm <- ctModel(LAMBDA = diag(1), Tpoints = Tpoints, DRIFT = matrix(-.5),
                    T0MEANS = matrix(4), CINT = matrix(par[subi]),
                    DIFFUSION = matrix(2), T0VAR = matrix(2),
                    MANIFESTVAR = matrix(2))
      d <- suppressMessages(ctGenerate(gm, n.subjects = 1, burnin = 10,
                                       wide = FALSE, dtmean = dt))
      if (subi == 1) dat <- cbind(subi, d) else dat <- rbind(dat, cbind(subi, d))
    }
    ...
    for (m in c("cm", "dm")) {
      argslist <- list(ml = list(datalong = dat, ctstanmodel = get(m), optimize = TRUE,
                                 nlcontrol = list(), verbose = 0,
                                 optimcontrol = list(plotsgd = F, estonly = F, stochastic = F),
                                 savescores = F, nopriors = T),
                       mlnl = list(datalong = dat, ctstanmodel = get(m), optimize = TRUE,
                                   nlcontrol = list(nldynamics = TRUE, nlmeasurement = TRUE), verbose = 0,
                                   optimcontrol = list(plotsgd = F, estonly = F, stochastic = F),
                                   savescores = F, nopriors = T))
