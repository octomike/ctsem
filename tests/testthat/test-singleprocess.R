if (.Machine$sizeof.pointer != 4) {
  ...
  test_that("singleprocess + reshape", {
    ...
    dtmat <- matrix(exp(rnorm(Tpoints - 1 + burnin, .3, .1)), byrow = TRUE,
                    nrow = nsubjects, ncol = Tpoints - 1 + burnin)
    genm <- ctModel(Tpoints = Tpoints, n.latent = n.latent,
    ...
    cd <- ctGenerate(ctmodelobj = genm, n.subjects = nsubjects, burnin = burnin,
                     dtmean = 1, wide = TRUE, dtmat = dtmat)
    ...
    sm <- ctModel(Tpoints = Tpoints, type = "stanct", n.latent = n.latent,
                  n.manifest = n.manifest, CINT = matrix("cint"),
                  MANIFESTVAR = diag(2, 1), MANIFESTMEANS = matrix(0),
                  LAMBDA = diag(1, n.manifest))

    sf <- ctStanFit(datalong = long, ctstanmodel = sm, nopriors = TRUE,
                    control = list(max_treedepth = 8), iter = 300,
                    chains = 2, plot = FALSE, fit = TRUE, optimize = TRUE)

    m <- ctModel(Tpoints = Tpoints, type = "omx", n.latent = n.latent,
                 n.manifest = n.manifest, CINT = matrix("cint"),
                 MANIFESTVAR = diag(2, 1), TRAITVAR = "auto",
    ...
    expect_equal(mean(sf$stanfit$transformedpars$DRIFT[,, 1, 1]), # nolint
                 summary(f)$ctparameters[2, 1],
                 tolerance = 1e-1)
