if (.Machine$sizeof.pointer != 4) {
  ...
  test_that("simplenonlinearcheck", {
    ...
    ssfitnl <- ctStanFit(datalong, ssmodel, iter = 300, chains = 1,
                         optimize = T, verbose = 0, maxtimestep = .3,
                         nlcontrol = list(nldynamics = TRUE),
                         optimcontrol = list(finishsamples = 10),
                         nopriors = TRUE, deoptim = FALSE)
    ssfitl <- ctStanFit(datalong, ssmodel, iter = 300, chains = 1,
                        optimize = T, verbose = 0,
                        nlcontrol = list(nldynamics = FALSE),
                        optimcontrol = list(finishsamples = 10, deoptim = FALSE),  # FIXME: wrong param position?
                        nopriors = TRUE)

    ssfitnlm <- ctStanFit(datalong, ssmodel, iter = 300, chains = 1,
                          optimize = T, verbose = 0, maxtimestep = 2,
                          fit = T,
                          # forcerecompile=T,
                          nlcontrol = list(nldynamics = T, nlmeasurement = T),
                          optimcontrol = list(finishsamples = 10),
                          nopriors = TRUE, deoptim = FALSE)
    ...
    expect_equal(round(ssfitnl$stanfit$transformedpars_old[grep("pop_", rownames(ssfitnl$stanfit$transformedpars_old)), "mean"], 3),
                 round(ssfitl$stanfit$transformedpars_old[grep("pop_", rownames(ssfitnl$stanfit$transformedpars_old)), "mean"], 3),
                 tol = .01)
