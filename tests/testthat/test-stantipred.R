if (identical(Sys.getenv("NOT_CRAN"), "true") & .Machine$sizeof.pointer != 4) {
  ...
  test_that("simpleTIpredcheck", {
    ...
    checkm <- ctModel(type = "stanct", Tpoints = Tpoints,
                      ...
                      n.TIpred = n.TIpred, n.manifest = n.manifest,
    ...
    tfit <- ctStanFit(tdat, checkm, chains = 2, optimize = T,
                      optimcontrol = list(is = TRUE, finishsamples = 200),
                      nopriors = F, verbose = 0)
    s <- summary(tfit)

    expect_equal(s$tipreds[2, "mean"], 5, tolerance = .1)

    tfit <- ctStanFit(tdat, checkm, iter = 400, warmup = 380, chains = 2,
                      optimize = T, optimcontrol = list(is = FALSE), nopriors = F,
                      verbose = 0, nlcontrol = list(nldynamics = TRUE, nlmeasurement = TRUE))
    s <- summary(tfit)
    expect_equal(s$tipreds[2, "mean"], 5, tolerance = .1)

    tfit <- ctStanFit(tdat, checkm, iter = 400, chains = 2,
                      control = list(adapt_delta = .8, max_treedepth = 6))
    s <- summary(tfit)
    expect_equal(s$tipreds[2, "mean"], 5, tolerance = .1)
  })
}
