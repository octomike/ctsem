if (identical(Sys.getenv("NOT_CRAN"), "true") & .Machine$sizeof.pointer != 4) {
  ...
  test_that("varyingLAMBDA", {
    ...
    for (subi in 1:nsubjects) {
      gm <- ctModel(LAMBDA = diag(2), Tpoints = Tpoints, DRIFT = diag(-.2, 2),
                    T0MEANS = matrix(c(3, 2)), DIFFUSION = diag(.5, 2),
                    T0VAR = diag(2))
    ...
    cm <- ctModel(LAMBDA = matrix(c("1 + PARS[1,1] * state[2]", 0, 0, 1), 2, 2),
                  PARS = matrix("lbystate"), type = "stanct")

    cm$pars$indvarying <- FALSE
    # cm$pars$indvarying[cm$pars$matrix %in% c("CINT", "T0MEANS")] <- TRUE

    dm <- ctModel(LAMBDA = matrix(c("1 + PARS[1,1] * state[2]", 0, 0, 1), 2, 2),
                  PARS = matrix("lbystate"), type = "standt")
    ...
    dtpars <- lapply(s, function(argi) {
      ...
      dt <- argi$dm$parmatrices[c(grep("(^asym)", rownames(argi$dm$parmatrices)),
                                ...
                                c("Mean", "Sd")]
      names <- rownames(dt) %in% c("DRIFT", "CINT", "DIFFUSIONcov")
      rownames(dt)[names] <- paste0("dt", names)
    ...
    for (dimi in 2:length(ll)) {
      ...
      sapply(s$ml, function(x) expect_equivalent(lambdafactor,
                                                 x$popmeans[rownames(x$popmeans) %in% "lbystate", "mean"],
                                                 tol = 1e-1))
