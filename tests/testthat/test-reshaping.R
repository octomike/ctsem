...
test_that("reshaping1", {
  ...
  generatingModel <- ctModel(Tpoints = Tpoints, n.latent = n.latent,
                             ...
                             #TDPREDMEANS = matrix(c(0, 0, 1, rep(0, (Tpoints - 1 - 3) * n.TDpred)),
  ...
  colnames(data) <- ctWideNames(n.manifest = n.manifest, n.TDpred = n.TDpred,
                                Tpoints = Tpoints, manifestNames = manifestNames,
                                TDpredNames = TDpredNames, TIpredNames = TIpredNames,
                                n.TIpred = n.TIpred)

  testlong <- ctWideToLong(data, n.manifest = n.manifest, n.TDpred = n.TDpred,
                           n.TIpred = n.TIpred, manifestNames = manifestNames,
                           TDpredNames = TDpredNames, TIpredNames = TIpredNames,
                           Tpoints = Tpoints)

  testlong <- ctDeintervalise(testlong)

  testwide <- ctLongToWide(testlong, id = "id", time = "time", manifestNames = manifestNames,
                           TDpredNames = TDpredNames, TIpredNames = TIpredNames)

  testwide <- ctIntervalise(testwide, n.manifest = n.manifest,
                            n.TDpred = n.TDpred, n.TIpred = n.TIpred,
                            Tpoints = Tpoints, manifestNames = manifestNames,
                            TDpredNames = TDpredNames, TIpredNames = TIpredNames)
