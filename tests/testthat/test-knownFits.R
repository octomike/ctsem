...
test_that("anomauth", {
  ...
  if (.Machine$sizeof.pointer != 4) {
    ...
    sf <- ctStanFit(ctDeintervalise(ctWideToLong(AnomAuth, Tpoints = AnomAuthmodel$Tpoints, n.manifest = 2)),
                    ctstanmodel = sm, optimize = TRUE, verbose = 0, savescores = FALSE,
                    cores = 2, nopriors = TRUE, optimcontrol = list(finishsamples = 10))
...
test_that("AnomAuth_traitasymptoticcheck", {
  ...
  data(AnomAuth)
  AnomAuthmodel <- ctModel(LAMBDA = matrix(c(1, 0, 0, 1), nrow = 2, ncol = 2),
                           n.latent = 2, n.manifest = 2, MANIFESTVAR = diag(0, 2),
                           TRAITVAR = "auto", CINT = matrix(0, nrow = 2),
                           T0MEANS = matrix(0, nrow = 2),
                           MANIFESTMEANS = matrix(c("m1", "m2"), nrow = 2),
                           Tpoints = 5)

  AnomAuthfit1 <- ctRefineTo(AnomAuth, AnomAuthmodel, asymptotes = FALSE,
                             verbose = 0, retryattempts = 1)
  AnomAuthfit2 <- ctRefineTo(AnomAuth, AnomAuthmodel, asymptotes = TRUE,
                             verbose = 0, retryattempts = 1)

  expect_equal(AnomAuthfit2$mxobj$output$Minus2LogLikelihood,
               AnomAuthfit1$mxobj$output$Minus2LogLikelihood)

  summ1 <- summary(AnomAuthfit1, verbose = TRUE)
  summ2 <- summary(AnomAuthfit2, verbose = TRUE)

  expect_equal(summ1$ctparameters[, 1:2],
               summ2$ctparameters[, 1:2],
               tolerance = .001)
})
...
test_that("oscillator", {
  ...
  oscillatingf <- ctFit(Oscillating, oscillatingm,
                        carefulFit = FALSE, retryattempts = 3)

  expect_equal(-3461.936,
               oscillatingf$mxobj$output$Minus2LogLikelihood,
               tolerance = .001)
  ...
  if (.Machine$sizeof.pointer != 4) {
    ...
    sf <- ctStanFit(ctDeintervalise(ctWideToLong(Oscillating, Tpoints = oscillatingm$Tpoints, n.manifest = 1)),
                    cores = 1, verbose = 0, ctstanmodel = sm, optimize = TRUE,
                    savescores = FALSE, nopriors = TRUE,
                    optimcontrol = list(finishsamples = 10))
    expect_equal(-3461.936,
                 -2 * sf$stanfit$optimfit$value,
                 tolerance = .01)
  }
})
}
