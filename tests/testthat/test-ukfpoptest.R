if (identical(Sys.getenv("NOT_CRAN"), "true") & .Machine$sizeof.pointer != 4) {
  ...
  test_that("ukfpopcheck1", {
    ...
    for (i in 1:nsubjects) {
      gm <- ctModel(type = "omx", n.latent = 2,
                    n.manifest = n.manifest, Tpoints = Tpoints,
                    LAMBDA = matrix(c(1, .4, 0, 0, 0, 1), 3, ncol = 2),
                    DRIFT = diag(-.4, 2),
      ...
      if (i == 1) {
        cd <- ctGenerate(gm, n.subjects = 1, burnin = burnin, wide = FALSE, dtmat = dtmat)
      } else {
    ...
    m1 <- ctModel(type = "omx", n.latent = 4,
                  n.manifest = 3, Tpoints = Tpoints,
    ...
    cfit2 <- ctRefineTo(dat = cd, dataform = "long", ctmodelobj = m2,
                        retryattempts = 0, carefulFit = T, stationary = "")
    ...
    # sink(file='../sf1.txt')
    sf1 <- ctStanFit(cd, sm1, iter = 200, optimize = TRUE, verbose = 0, nopriors = TRUE,
                     optimcontrol = list(deoptim = F, is = FALSE, isloopsize = 50, finishsamples = 50),
                     derrind = 1:4, nlcontrol = list(nldynamics = FALSE))
    # sink()
    # summary(sf1)

    sf1d <- matrix(sf1$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                        rownames(sf1$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
    sf1ll <- sf1$stanfit$optimfit$value

    sf1_derrind <- ctStanFit(cd, sm1, iter = 200, optimize = TRUE, verbose = 0, nopriors = TRUE,
                             optimcontrol = list(deoptim = FALSE, is = FALSE, isloopsize = 50, finishsamples = 50),
                             derrind = 1:2, nlcontrol = list(nldynamics = FALSE))
    sf1_derrindd <- matrix(sf1_derrind$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                                        rownames(sf1_derrind$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
    sf1_derrindll <- sf1_derrind$stanfit$optimfit$value

    sf1nld <- ctStanFit(cd, sm1, iter = 200, optimize = TRUE, verbose = 0, nopriors = TRUE,
                        optimcontrol = list(deoptim = FALSE, is = FALSE, isloopsize = 50, finishsamples = 50),
                        derrind = 1:4, nlcontrol = list(nldynamics = TRUE))
    sf1nldd <- matrix(sf1nld$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                              rownames(sf1nld$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
    sf1nldll <- sf1nld$stanfit$optimfit$value

    # sink(file='../sinkoutg.txt')
    sf1nld_derrind <- ctStanFit(cd, sm1, iter = 200, optimize = TRUE, verbose = 0, nopriors = TRUE, init = 0,
                                optimcontrol = list(deoptim = FALSE, is = FALSE, isloopsize = 50, finishsamples = 50),
                                derrind = 1:2, nlcontrol = list(nldynamics = TRUE))
    # sink()
    sf1nld_derrindd <- matrix(sf1nld_derrind$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                                              rownames(sf1nld_derrind$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
    sf1nld_derrindll <- sf1nld_derrind$stanfit$optimfit$value


    sf1nl_derrind <- ctStanFit(cd, sm1, iter = 200, optimize = TRUE, verbose = 0, nopriors = TRUE,
                               optimcontrol = list(deoptim = FALSE, is = FALSE, isloopsize = 50, finishsamples = 50),
                               derrind = 1:2, nlcontrol = list(nldynamics = TRUE, nlmeasurement = TRUE))
    sf1nl_derrindd <- matrix(sf1nl_derrind$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                                            rownames(sf1nl_derrind$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
    ...
    sf2 <- ctStanFit(cd, sm2, iter = 10, fit = T, optimize = T, verbose = 0, nopriors = TRUE, intoverpop = TRUE,
                     # init=0,
                     optimcontrol = list(deoptim = FALSE, is = FALSE, isloopsize = 50, finishsamples = 50),
                     derrind = 1:2)
    # sink()
    sf2d <- matrix(sf2$stanfit$transformedpars_old[grep("pop_DRIFT",
                                                        rownames(sf2$stanfit$transformedpars_old)), "mean"], 4, 4)[1:2, 1:2]
  ...
  test_that("ukfpopcheck2_randomdrift", {
    ...
    for (si in 1:nsubjects) {
      m <- ctModel(LAMBDA = diag(1), Tpoints = Tpoints, DRIFT = matrix(drift[si]),
                   T0MEANS = matrix(3), T0VAR = matrix(sqrt(.1)), DIFFUSION = diag(1, 1),
                   CINT = matrix(-2), MANIFESTVAR = matrix(sqrt(.1)))
    ...
    dtm <- ctModel(LAMBDA = diag(1), type = "stanct",
                   ...
                   CINT = matrix("cint"), MANIFESTMEANS = matrix(0))
    ...
    dtf <- ctStanFit(datalong = dat, ctstanmodel = dtm, optimize = TRUE,
                     cores = 1, verbose = 0, optimcontrol = list(estonly = T),
                     savescores = F, nopriors = T)

    dtm2 <- ctModel(LAMBDA = matrix(c(1, 0), 1, 2), type = "stanct",
                    DIFFUSION = matrix(c("diff", 0, 0, 0), 2, 2),
                    DRIFT = matrix(c("-2*log1p(exp(2*state[2]))", 0, 0, -.00001), 2, 2),
                    CINT = matrix(c("cint", 0), 2, 1), T0MEANS = matrix(c("t0m1", "t0m2|param"), 2, 1),
                    T0VAR = matrix(c("t0v11", 0, 0, paste0("t0var22 | ",
                                                           gsub(".* sdscale", "",
                                                                gsub("rawpopsdbase", "param", dtm$rawpopsdtransform),
                                                                fixed = TRUE))), 2, 2),
                    MANIFESTMEANS = matrix(0))

    dtm2$pars$indvarying <- FALSE
    dtf2 <- ctStanFit(datalong = dat, ctstanmodel = dtm2, optimize = TRUE,
                      optimcontrol = list(estonly = T), savescores = F, nopriors = T)
