...
model1 <- ctModel(type = "omx", n.latent = 2, n.manifest = 1, Tpoints = TT,
                  ...
                  DRIFT = matrix(c(0,  1, -.3, -.35), nrow = 2, ncol = 2, byrow = TRUE),
                  MANIFESTMEANS = matrix(c(46), nrow = 1, ncol = 1),
                  MANIFESTVAR = matrix(c(0), nrow = 1, ncol = 1),
                  CINT = matrix(c(0, 0), nrow = 2, ncol = 1),
                  T0MEANS = matrix(c(0, 0), nrow = 2, ncol = 1),
                  T0VAR = diag(10, 2), PARS = matrix("p"),
                  DIFFUSION = matrix(c(0, 0, 0, 16), ncol = 2, nrow = 2, byrow = TRUE))
fit1 <- ctFit(dat = datalong, dataform = "long", ctmodelobj = model1,
              transformedParams = F, fit = F, objective = "Kalman",
              stationary = "T0VAR", carefulFit = F)
